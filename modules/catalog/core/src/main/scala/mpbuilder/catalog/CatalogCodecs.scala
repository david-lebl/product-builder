...
/** JSON codecs for catalog types (materials, finishes, specifications,
  * configurations, presets, compatibility rules).
  *
  * Re-exports the kernel codecs, so a single
  * {{{
  * import mpbuilder.catalog.CatalogCodecs.given
  * }}}
  * brings everything needed to serialize catalog types.
  */
object CatalogCodecs:

  // ── Shared-kernel codecs (IDs, Language, Money) ──────────────────────────

  export mpbuilder.kernel.KernelCodecs.given
  ...
  given JsonCodec[FinishParameters.RopeParams] = DeriveJsonCodec.gen[FinishParameters.RopeParams]
