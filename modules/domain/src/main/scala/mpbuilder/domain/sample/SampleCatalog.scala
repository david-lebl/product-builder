...
object SampleCatalog:
  ...
  val pvc510g: Material = Material(
    ...
    family = MaterialFamily.Vinyl,
    weight = Some(PaperWeight.unsafe(510)),
    properties = Set(MaterialProperty.WaterResistant, MaterialProperty.SmoothSurface),
    description = Some(LocalizedString(
      "Heavy-duty PVC-coated banner fabric (510 g/m²). The standard material for outdoor and indoor banners. " +
        "Weather-resistant, UV-stable, and suitable for grommets. Maximum in-house printing width 150 cm.",
      "Odolná PVC bannerová tkanina (510 g/m²). Standardní materiál pro venkovní i interiérové bannery. " +
        "Odolný povětrnostním vlivům, UV stabilní, vhodný pro průchodky. Maximální šířka tisku v rámci vlastní výroby 150 cm.",
    )),
  )
  ...
  val gumRope: Finish = Finish(
    ...
    side = FinishSide.Both,
    description = Some(LocalizedString(
      "Elastic rubber rope threaded through grommets to keep the banner stretched taut. Priced per meter of rope. Requires grommets to be present.",
      "Elastické gumové lano protažené průchodkami k udržení banneru napnutého. Cena za metr lana. Vyžaduje přítomnost průchodek.",
    )),
  )
  ...
  val banners: ProductCategory = ProductCategory(
    ...
    description = Some(LocalizedString(
      "Large-format PVC banners for outdoor and indoor use. Printed with UV-curable inks for weather resistance. " +
        "Optional grommets for hanging, gum rope for tensioning, and die-cutting for custom shapes. Maximum in-house size 150 cm.",
      "Velkoformátové PVC bannery pro venkovní i vnitřní použití. Tištěné UV vytvrzovanými inkousty pro odolnost proti povětrnostním vlivům. " +
        "Volitelné průchodky pro zavěšení, gumové lano pro napnutí a výsek pro vlastní tvary. Maximální rozměr vlastní výroby 150 cm.",
    )),
    presets = List(
      CategoryPreset(
      ...
      CategoryPreset(
        ...
        description = Some(LocalizedString(
          "PVC 510g, 4+0 CMYK, UV coating + grommets, 1000×1500 mm, 1 pc",
          "PVC 510g, 4+0 CMYK, UV lak + průchodky, 1000×1500 mm, 1 ks",
        )),
        printingMethodId = uvInkjetId,
        componentPresets = List(ComponentPreset(
          role = ComponentRole.Main,
  ...
  private val allMaterialIds: Set[MaterialId] = Set(
    ...
    coatedSilk250gsmId, yupoId, adhesiveStockId, cottonId, clearVinylId, pvc510gId,
