...
/** Compact alternative to [[ProductBuilderApp]].
  *
  * Identical functionality, but uses a `compact-builder` CSS wrapper so that
  * textfield labels are rendered to the LEFT of each input/select (inline),
  * making the form considerably more space-efficient.
  *
  * Finish checkboxes are intentionally left unmodified — their label / subform
  * structure already has edge-case CSS (border-radius change when the param
  * subform expands), and further nesting would make those issues worse.
  */
object ProductBuilderCompactApp:
  def apply(): Element =
      ...
      div(
        ...
        div(
          ...
          h2(child.text <-- lang.map {
            case Language.En => "Product Parameters (Compact)"
            case Language.Cs => "Parametry produktu (Kompaktní)"
          }),
