...
- `ProductionCostRule` sealed ADT:
  ...
  - `MaterialAreaCost(materialId: MaterialId, costPerSqMeter: Money)`
...
- `CostWarning` ADT: `BelowProductionCost(shortfall: Money)`, `LowMargin(marginPct: Percentage, threshold: Percentage)` — with bilingual `message(lang)` method

**Service (`pricing/ProductionCostCalculator.scala`):**
- `calculateCost(config, costSheet): Validation[PricingError, Money]` — walks components, resolves material/finish/process costs, applies overhead factor
- `analyze(config, pricelist, costSheet, lowMarginThreshold): Validation[PricingError, CostAnalysis]` — computes selling price via `PriceCalculator`, compares with production cost, emits warnings

**Sample data (`sample/SampleProductionCosts.scala`):**
- Cost rules for sample materials (8 base-priced, 4 area-priced), 3 process types, 8 finishes
- 15% overhead factor
- `costSheetNoOverhead` variant for testing overhead impact

**Tests (`ProductionCostCalculatorSpec`):**
...
**Implementation notes:**
- `ProductionCostCalculator.calculateCost` mirrors `PriceCalculator` structure: walks components, resolves material costs (area > unit precedence), adds finish and process costs, applies overhead multiplier
- `ProductionCostCalculator.analyze` takes `pricelist` (base or customer-adjusted) — customer pricing integration is done by passing the customer-resolved pricelist, keeping the calculator decoupled
- `CostWarning.LowMargin` uses a configurable threshold (default 20%)
- Lamination side factor (`FinishSide.Both` = 2x) is applied to finish costs, consistent with `PriceCalculator`
- Materials/finishes not in the cost sheet contribute zero cost (graceful handling)
- 12 tests in `ProductionCostCalculatorSpec` covering: base-priced and area-priced materials, overhead factor, unknown materials, missing quantity, healthy margin, below-cost detection, low margin warning, custom threshold, customer discount impact, bilingual messages
...
| 3 — Production Cost | `ProductionCost`, `ProductionCostCalculator` | — | 12 | Phase 2 | ✅ Done |
